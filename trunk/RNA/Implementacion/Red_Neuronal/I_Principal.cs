...
namespace Red_Neuronal
{
    ...
    {
        //Variables globales
        private Control_BackPropagation control_bp;     //Objeto de control de la red neuronal retropropagacion
        private Control_CounterPropagation control_cp;  //Objeto de control de la red neuronal contrapropagacion
        ...
        private bool red_bp_establecida;                //Indica si la red de retroPropagacion ha sido establecida
        private bool red_cp_establecida;                //Indica si la red de contraPropagacion ha sido establecida
        ...
        public I_Principal()
        {
            ...
            control_bp = new Control_BackPropagation();
        ...
        private void menu_entrenamiento_Click(object sender, EventArgs e)
        {
            ...
            red_cp_establecida = false;
            limpiar_matriz();
            campo_ruta_pesos_ejecucion_bp.Text = campo_ruta_pesos_ejecucion_cp.Text = "";
        ...
        private void boton_estabalecer_pesos_ejecucion_bp_Click(object sender, EventArgs e)
        {
            if (campo_ruta_pesos_ejecucion_bp.Text.EndsWith(".ap"))            //Si contiene la extension correcta
            {
                ...
                {
                    if (control_bp.configurar_red_ejecucion(campo_ruta_pesos_ejecucion_bp.Text))//Configura la red para la ejecucion
                    {
                        red_bp_establecida = true;                              //Se establecio la red              
                        boton_estabalecer_pesos_ejecucion_bp.Enabled = false;
                    }
                    else                                                        //Si se produce un error al establecer la configuracion de la red
                    {
                        red_bp_establecida = false;                             //No se establecio la redgroupBox_Dibujo.Enabled = false;                        //Deshabilita el panel de dibujo
            ...
            habilitar_reconocimiento();                                     //Realiza la habilitacion del reconocimiento
        }
        ...
        private void boton_entrenar_Click(object sender, EventArgs e)
        {
            if (campo_ruta_muestras_bp.Text.EndsWith(".cm"))              //Si contiene la extension correcta
            {
                if (File.Exists(campo_ruta_muestras_bp.Text))
                {
                    ...
                    control_bp.configurar_red_aprendizaje(Convert.ToInt32(campo_neuronas_entrada_bp.Value), Convert.ToInt32(campo_neuronas_intermedia_bp.Value), Convert.ToInt32(campo_neuronas_salida_bp.Value), Convert.ToDouble(campo_IniPesos_Hasta_bp.Value), Convert.ToDouble(campo_IniPesos_Desde_bp.Value));
                    iteraciones = 0;
                    error_grafica = 0.0;
                    cont_entrenar = true;
                    resultado_entrenamiento = 0;
        ...
        private void entrenar()
        {
            ...
            resultado_entrenamiento = control_bp.entrenar(Convert.ToDouble(campo_error_bp.Value), Convert.ToDouble(campo_aprendizaje_bp.Value), campo_ruta_muestras_bp.Text, ref iteraciones, ref error_grafica, ref cont_entrenar);
            ...
            {
                entrenamiento_finalizado("BP");     //Finaliza el entrenamiento
            ...
            else
            {
                if (resultado_entrenamiento == 2)   //Si se decidio abortar
                {
                    ...
                    salvar_pesos("BP"); //Envia a guardar los pesos de entrenamiento
        ...
        private void panel_dibujo_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < 15; i++)                        //Recorre filas
            {
                for (int j = 0; j < 15; j++)                    //Recorre columnas
                {
                    ...
                    {
                        brocha.Color = (Color.Maroon);  //Cambia el color de la brocha
                    }
                    else
                    {
        ...
        {
            //Reconoce en la red de retropropagacion
            Thread p1;                                          
            p1 = new Thread(new ThreadStart(this.reconocer_bp));
            p1.SetApartmentState(ApartmentState.STA);
            p1.IsBackground = true;
            p1.Start();

            //Reconoce en la red de contrapropagacion
            Thread p2;                                          
            p2 = new Thread(new ThreadStart(this.reconocer_cp));
            p2.SetApartmentState(ApartmentState.STA);
            p2.IsBackground = true;
            p2.Start(); 
        }
        ...
        /// <summary>
        /// Realiza el reconocimiento en la red de retropropagacion
        /// </summary>
        private void reconocer_bp()
        {
            ...
            int[] resultado = control_bp.reconocer(entrada);                        //Realiza la ejecucion y guarda el resultado
        ...
        /// <summary>
        /// Realiza el reconocimiento en la red de contrapropagacion
        /// </summary>
        private void reconocer_cp()
        {
            int[] entrada = new int[225];
            for (int i = 0; i < 15; ++i)                    //recorre las filas de la matriz de dibujo
            {
            ...
            DateTime inicio = DateTime.Now;                                         //Guarda la hora a la que inicio el reconocimiento
            int[] resultado = control_cp.reconocer(entrada);                        //Realiza la ejecucion y guarda el resultado
            DateTime final = DateTime.Now;                                          //Guarda la hora en la que finaliza el reconocimiento
            TimeSpan total = final - inicio;                                        //Calcula el tiempo total
            campo_tiempo_ejecucion_cp.Text = total.TotalSeconds.ToString();         //Muestra el tiempo total
            campo_resultado_ejecucion_cp.BackgroundImage = imagenes_figuras.Images[reconocer_codificacion(resultado)];//Coloca el resultato del procesamiento
        }
        ...
        private void boton_guardar_muestras_Click(object sender, EventArgs e)
        {
            if (guardar_muestras)   //Si se pueden guardar las muestras
            {
                if (ventana_salvar_muestras.ShowDialog() == DialogResult.OK)            //Si se realizo click en ok
                {
                    ...
                    {
                        if (control_bp.guardar_muestras(ventana_salvar_muestras.FileName, muestras_creadas))
                        {
                            muestras_creadas = "";                                          //Borra las muestras creadas
        ...
        /// </summary>
        /// <param name="tipo_red">nombre del tipo de la red que entreno</param>
        public void entrenamiento_finalizado(String tipo_red)
        {
            ...
            salvar_pesos(tipo_red); //Envia a guardar los pesos de entrenamiento
        }

        /// <summary>
        /// Gestiona el guardado de los pesos de entrenamiento
        /// </summary>
        /// <param name="tipo_red">nombre del tipo de la red que entrenó</param>
        private void salvar_pesos(String tipo_red)
        {
            ...
            if (ventana_salvar_pesos.ShowDialog() == DialogResult.OK)   //Si se realizo click en ok
            {
                ...
                {
                    bool res;                                           //Guarda el resultado de guardar los pesos 
                    if (tipo_red == "BP")                               //Si es de retropropagacion                        
                    {
                        res = control_bp.guardar_pesos(ventana_salvar_pesos.FileName);
                    }
                    else                                                //Si es de contrapropagacion                        
                    {
                        res = control_cp.guardar_pesos(ventana_salvar_pesos.FileName);
                    }

                    if (res)  //Si se guardo correctamente
                    {
        ...
        /// <summary>
        /// Se encarga invocar el entrenamiento de la red neuronal de contrapropagacion
        /// </summary>
        private void entrenar_cp()
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Se ha forzado la finalización del entrenamiento.\nA continuación puede guardar los pesos que se tienen  hasta el momento", "Entrenamiento abortado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    salvar_pesos("CP"); //Envia a guardar los pesos de entrenamiento
                    menu_entrenamiento_Click(null, null);
                }
                else //Si el entrenamiento salio con error
                {
        ...
        private void boton_estabalecer_pesos_ejecucion_cp_Click(object sender, EventArgs e)
        {
            if (campo_ruta_pesos_ejecucion_cp.Text.EndsWith(".ap"))            //Si contiene la extension correcta
            {
                if (File.Exists(campo_ruta_pesos_ejecucion_cp.Text))           //Si el archivo existe
                {
                    ...
                    {
                        red_cp_establecida = false;                             //no se establecio la red
                        MessageBox.Show("Se produjo un error al establecer la configuración de la red", "Error configuración de red", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
        ...
        /// <summary>
        /// Habilita o deshabilita el panel de reconocimiento dependiendo del estado de las redes
        /// </summary>
        private void habilitar_reconocimiento()
        {
            if (red_bp_establecida && red_cp_establecida)       //Si ambas redes se han establecido
            {
