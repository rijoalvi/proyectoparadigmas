...
namespace Red_Neuronal
{
    ...
    class Control_CounterPropagation
    {
        ...
        public int entrenar(double c_aprendizaje_alpha, double c_aprendizaje_beta, String ruta_muestras, ref int num_iteracion, ref bool cont_entrenar)
        {
            try
            {
        ...
        /// </summary>
        /// <returns>Valor del ganador de la capa oculta</returns>
        private double calcular_ganador()
        {
            int ganador = 0;            //El indice de la neurona que tiene la salida mayor (ganadora)
            for (int h = 0; h < red_neuronal.get_cantidad_neuronas_entrada(); h++) //Sumatoria de los valores netos
            {
                ...
                { // compara cual es el ganador de la capa oculta
                    ganador = h;
                    red_neuronal.set_valor_oculta(h, bin_cero);
                }
                else if ((red_neuronal.get_valor_oculta(h) == red_neuronal.get_valor_oculta(ganador)) && (ganador != h))
                {
                    red_neuronal.set_valor_oculta(h, bin_cero);
                }
            }
            if (ganador != 0)
            {
                red_neuronal.set_valor_oculta(0, bin_cero);
            }
            red_neuronal.set_valor_oculta(ganador, bin_uno);
            return ganador;
        }
