...
namespace Red_Neuronal
{
    ...
    {
        //Variables globales
        private Control control;                    //Objeto de control de la red neuronal
        ...
        {
            panel_muestra_entrenamiento.Visible = panel_ejecucion.Visible = panel_crear_muestras.Visible = panel_entrenamiento.Visible = false;
        ...
        {
            panel_ejecucion.Visible = panel_crear_muestras.Visible = panel_muestra_entrenamiento.Visible= false;        //Habilita y deshabilita los botones correspondientes
            panel_entrenamiento.Visible = true;
            menu_entrenamiento.Enabled = false;
            menu_ejecucion.Enabled = true;
            limpiar_matriz();
            campo_ruta_pesos_ejecucion.Text = "";
        ...
        private void menu_ejecucion_Click(object sender, EventArgs e)
        {
            ...
            panel_entrenamiento.Visible = panel_crear_muestras.Visible = panel_muestra_entrenamiento.Visible = false;
            menu_entrenamiento.Enabled = true;
            menu_ejecucion.Enabled = false;
            limpiar_matriz();
            campo_tiempo_ejecucion.Text = campo_resultado_ejecucion.Text = campo_ruta_pesos_ejecucion.Text = "";
        }
        ...
        private void boton_elegir_ruta_patrones_Click(object sender, EventArgs e)
        {
            ...
            {
                campo_ruta_muestras.Text = ventana_elegir_ruta_muestras.FileName;   //Coloca la ruta del archivo
            }
        }
        ...
        {
            if (campo_ruta_muestras.Text != "")     //Si hay una ruta en el campo
            {
                boton_entrenar.Enabled = true;      //Habilita la opcion de entrenar
            }
            else                                    //Sino
            {
                boton_entrenar.Enabled = false;     //Deshabilita la opcion de entrenar
            }
        }
        ...
        private void boton_elegir_pesos_ejecucion_Click(object sender, EventArgs e)
        {
            ...
            {
                campo_ruta_pesos_ejecucion.Text = veantana_elegir_ruta_pesos.FileName;  //Coloca la ruta del archivo
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void campo_ruta_pesos_ejecucion_TextChanged(object sender, EventArgs e)
        {
            if (campo_ruta_pesos_ejecucion.Text != "")              //Si hay una ruta en el campo
            {
                boton_estabalecer_pesos_ejecucion.Enabled = true;   //Habilita la opcion de establecer
            }
            else                                                    //Sino
            {
                boton_estabalecer_pesos_ejecucion.Enabled = false;  //Deshabilita la opcion de establecer
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void boton_estabalecer_pesos_ejecucion_Click(object sender, EventArgs e)
        {
            if (campo_ruta_pesos_ejecucion.Text.EndsWith(".ap"))            //Si contiene la extension correcta
            {
                if (File.Exists(campo_ruta_pesos_ejecucion.Text))           //Si el archivo existe
                {
                    if (control.configurar_red_ejecucion(campo_ruta_pesos_ejecucion.Text))//Configura la red para la ejecucion
                    {
                        groupBox_Dibujo.Enabled = true;                         //Habilita el panel de dibujo            
                        boton_estabalecer_pesos_ejecucion.Enabled = false;
                    }
        ...
        {
            if (campo_ruta_muestras.Text.EndsWith(".cm"))              //Si contiene la extension correcta
            {
                if (File.Exists(campo_ruta_muestras.Text))
                {
                    panel_muestra_entrenamiento.Visible = true;
                    control.configurar_red_aprendizaje(Convert.ToInt32(campo_neuronas_entrada.Value), Convert.ToInt32(campo_neuronas_intermedia.Value), Convert.ToInt32(campo_neuronas_salida.Value), Convert.ToDouble(campo_IniPesos_Hasta.Value), Convert.ToDouble(campo_IniPesos_Desde.Value));
                    iteraciones = 0;
                    error_grafica = 0.0;
                    cont_entrenar = true;
                    resultado_entrenamiento = 0;
        ...
        private void entrenar()
        {
            ...
            resultado_entrenamiento = control.entrenar(Convert.ToDouble(campo_error.Value), Convert.ToDouble(campo_aprendizaje.Value), campo_ruta_muestras.Text, ref iteraciones, ref error_grafica, ref cont_entrenar);
        ...
        private void boton_limpiar_panel_Click(object sender, EventArgs e)
        {
            ...
            campo_resultado_ejecucion.Text = campo_tiempo_ejecucion.Text = "";
        }
        ...
        private void boton_reconocer_figura_Click(object sender, EventArgs e)
        {
            ...
            TimeSpan total = final - inicio;                                    //Calcula el tiempo total
            campo_tiempo_ejecucion.Text = total.TotalSeconds.ToString();        //Muestra el tiempo total
            campo_resultado_ejecucion.Text = reconocer_codificacion(resultado); //Obtiene la figura resultante segun la codificacion
        }
        ...
        {
            panel_ejecucion.Visible = panel_entrenamiento.Visible = panel_muestra_entrenamiento.Visible = false;
        ...
        /// <returns>La figura codificada, ´?´ si no pudo codificar</returns>
        private String reconocer_codificacion(int[]entrada)
        {
            //Retorna la configuracion segun la figura
            ...
            if (guardar_muestras)   //Si se pueden guardar las muestras
            {
                if (ventana_salvar_muestras.ShowDialog() == DialogResult.OK)            //Si se realizo click en ok
                {
                    if (ventana_salvar_muestras.FileName.EndsWith(".cm"))               //Si contiene la extension correcta
                    {
                        ...
                        {
                            muestras_creadas = "";                                          //Borra las muestras creadas
                            campo_ruta_muestras.Text = ventana_salvar_muestras.FileName;    //Coloca la ruta del archivo
                            menu_entrenamiento_Click(null, null);                           //Muestra el panel de entrenamiento
                        }
                        else
                        {
        ...
        private void panel_muestra_entrenamiento_VisibleChanged(object sender, EventArgs e)
        {
            ...
            if (panel_muestra_entrenamiento.Visible == true)
            {
            ...
            if (ventana_salvar_pesos.ShowDialog() == DialogResult.OK)                       //Si se realizo click en ok
            {
                if (ventana_salvar_pesos.FileName.EndsWith(".ap"))                          //Si contiene la extension correcta
                {
                    if (control.guardar_pesos(ventana_salvar_pesos.FileName))      //Si se guardo correctamente
                    {
                        ...
                        campo_ruta_pesos_ejecucion.Text = ventana_salvar_pesos.FileName;
                    }
                    else
                    {
